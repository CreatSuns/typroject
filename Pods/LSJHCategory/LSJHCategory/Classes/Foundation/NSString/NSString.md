...
## 说明
- 系统方法的扩展二次封装会用<font color="red">前缀ll_safe</font>+原始的API名字命名
...
- (NSUInteger)ll_safeReplaceOccurrencesOfString:(NSString *)target withString:(NSString *)replacement options:(NSStringCompareOptions)options range:(NSRange)searchRange;
```
## NSString+Category
...
/**
 ...
 @return 大小
 */
- (unsigned long long)ll_fileSize;
 ...
 */
+ (NSInteger)ll_compareVersion:(NSString *)v1 to:(NSString *)v2;
```
